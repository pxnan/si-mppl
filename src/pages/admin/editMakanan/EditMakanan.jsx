...
const EditMakanan = () => {
  ...
  const fetchMakananData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/getMakananById/${id}`
      );
  ...
  const handleFileChange = (event) => {
    ...
    setMakananData({ ...makananData, gambar: URL.createObjectURL(file) });
  };
  ...
  const onSubmit = async (event) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:5000/editMakanan/${id}`,
        formData,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        navigate("/dashboard/listmakanan/")
      }
  ...
  return (
    <AdminLayout>
      <div className="p-4">
        <h1 className="text-3xl font-bold mb-4">Edit Makanan</h1>
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div className="form-control">
            ...
            <textarea
              ...
              rows="4"
              defaultValue={makananData.penjelasan}
              required
            ></textarea>
          </div>
          ...
          <div className="form-control flex flex-row gap-6 justify-center">
            <Link to={"/dashboard/listmakanan"} className="btn btn-error w-2/5">
              Cancel
            </Link>
